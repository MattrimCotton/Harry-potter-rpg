...
| `sonar-project.properties` | Configuration SonarQube pour l'analyse de qualité (clé `MattrimCotton_Harry-potter-rpg`, org `mattrimcotton`) ; analyse `src/` et `tools/`, exclut `src/lib/` (Bootstrap) |
| `.mcp.json` | Serveur MCP SonarQube pour Claude Code (`sonar run mcp`, Docker requis) |
| `.github/workflows/sonarcloud.yml` | Analyse SonarCloud à chaque push sur `main` et à chaque pull request (secret `SONAR_TOKEN` requis) |
